...
def segmentMalignantRegions(image):
    ...
    adaptive_thresh_image = cv2.adaptiveThreshold(
        enhanced_image, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 843, 25
    )
    ...
    kernel = np.ones((7, 7), np.uint8)  # Create a smaller kernel for dilation
    mask = cv2.dilate(mask, kernel, iterations=1)  # Dilate the mask to ensure object boundaries are more pronounced
    ...
    num_labels, labels = cv2.connectedComponents(mask, connectivity=8)  # Find connected components in the binary mask

    # Optionally, filter out small components
    min_size = 2700  # Minimum size of components to retain
